...
RUN_SCRIPT="$ROOT/bin/OS/darwin/scripts/run.sh"
cd "$ROOT/bin"
...
clear_app_launch_quarantine() {
  ...
  xattr -cr "$RUN_SCRIPT" "$ROOT/bin/run_mac.sh" "$ROOT/venv/bin" 2>/dev/null || true
  chmod +x "$RUN_SCRIPT" "$ROOT/bin/run_mac.sh" 2>/dev/null || true
}
